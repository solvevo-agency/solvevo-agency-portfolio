    ...
    <header className="sticky top-0 z-50 w-full border-b bg-background/80 backdrop-blur-md">
      <div className="section-padding-x flex h-16 items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Mobile Nav Sheet */}
          <Sheet open={isMobileNavOpen} onOpenChange={(open) => !open ? dispatch(closeMobileNav()) : dispatch(toggleMobileNav())}>
            ...
            <SheetContent side="right" className="w-[300px]">
              ...
              <nav className="flex flex-col gap-4">
                  ...
                  return (
                    <Link
                      ...
                      className={`text-base font-medium py-2 transition-colors hover:text-primary ${
                        isActive ? "text-primary font-semibold border-l-2 border-primary pl-2" : "text-muted-foreground"
                      }`}
                    >
                      {link.label}
                    </Link>
                ...
                <Link
                  href="/#contact"
                  onClick={handleLinkClick}
                  className={cn(buttonVariants({ size: "default" }), "mt-4 text-center")}
                >
                  Get in Touch
                </Link>
              </nav>
